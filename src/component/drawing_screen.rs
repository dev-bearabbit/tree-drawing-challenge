...
pub fn drawing_screen(props: &DrawingScreenProps) -> Html {
    ...
    let throttle_duration = 30.0; // 약 33FPS (ms)

    let on_update_draw = {
        let on_update_draw = props.on_update_draw.clone();
        let last_update = last_update.clone();
        Callback::from(move |event: TouchEvent| {
            let now = Date::now();
            if now - *last_update > throttle_duration {
                on_update_draw.emit(event);
                last_update.set(now); // 상태 업데이트
            }
        })
    };
    ...
    let on_touch_end = {
        ...
        Callback::from(move |_| on_touch_end.emit(())) // 터치 종료 시 호출
    };
