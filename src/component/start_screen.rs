...
pub fn start_screen(props: &StartScreenProps) -> Html {
    html! {
        <div class="start-screen">

            <div class="sentence">
                <h1>
                    <span class="highlight">{"트리"}</span>{" 그리기 챌린지"}
                </h1>
                <p>{ "5초 안에 트리를 그려보세요!" }</p>
            </div>

            <div class="tree-container">
                <svg width="100%" height="auto" viewBox="0 0 200 400" preserveAspectRatio="xMidYMid meet" class="drawing_pattern">

                    <svg x="80" y="15" width="40" height="40" viewBox="0 0 52 49" preserveAspectRatio="xMidYMin" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <svg width="256" height="291" viewBox="0 0 256 291" preserveAspectRatio="xMidYMin" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="100" y="200" font-size="5vw" text-anchor="middle" fill="white" dominant-baseline="middle">
                        { "05 : 00" }
                    </text>
                </svg>
            </div>

            <button onclick={props.on_start.clone()} class="start-button">{ "도전하기" }</button>
        </div>
    }
}